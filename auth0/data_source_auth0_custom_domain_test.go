...
func init() {
	resource.AddTestSweepers("data_auth0_custom_domain", &resource.Sweeper{
		Name: "data_auth0_custom_domain",
...
`, rand),
				Check: resource.ComposeAggregateTestCheckFunc(
					random.TestCheckResourceAttr("data.auth0_custom_domain.my_custom_domain", "domain", "{{.random}}.auth.uat.alexkappa.com", rand),
					resource.TestCheckResourceAttr("data.auth0_custom_domain.my_custom_domain", "type", "auth0_managed_certs"),
					resource.TestCheckResourceAttr("data.auth0_custom_domain.my_custom_domain", "status", "pending_verification"),
				),
			},
		},
